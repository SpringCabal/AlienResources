...
local LOG_SECTION            = "Deferred rendering"
	...
	if options.enableHDR.value then
		...
		if options.enableBloom.value then
			...
			if not brightTexture1 or not brightTexture2 then
                Spring.Log(LOG_SECTION, LOG.ERROR, 'Deferred Rendering: Failed to create bloom buffers!')
				options.enableBloom.value = false
			end
		end

		if not screenHDR then
			Spring.Log(LOG_SECTION, LOG.ERROR, 'Deferred Rendering: Failed to create HDR buffer!')
...
function widget:Initialize()
	...
	if (glCreateShader == nil) then
		Spring.Log(LOG_SECTION, LOG.WARNING, 'Deferred Rendering requires shader support!')
	...
	if (Spring.GetConfigString("AllowDeferredMapRendering") == '0' or Spring.GetConfigString("AllowDeferredModelRendering") == '0') then
		Spring.Log(LOG_SECTION, LOG.WARNING, 'Deferred Rendering (gfx_deferred_rendering.lua) requires  AllowDeferredMapRendering and AllowDeferredModelRendering to be enabled in springsettings.cfg!')
		widgetHandler:RemoveWidget()
		return
	end
	if ((not forceNonGLSL) and Spring.GetMiniMapDualScreen() ~= 'left') then --FIXME dualscreen
		if (not glCreateShader) then
			Spring.Log(LOG_SECTION, LOG.ERROR, "gfx_deferred_rendering.lua: Shaders not found, removing self.")
			GLSLRenderer = false
			widgetHandler:RemoveWidget()
		else
			fragSrc = VFS.LoadFile("shaders/deferred_lighting.fs", VFS.ZIP)
			...
			if (not depthPointShader) then
				Spring.Log(LOG_SECTION, LOG.ERROR, glGetShaderLog())
				Spring.Log(LOG_SECTION, LOG.ERROR, "gfx_deferred_rendering.lua: Bad depth point shader, removing self.")
				GLSLRenderer = false
				widgetHandler:RemoveWidget()
			else
			...
			if (not depthBeamShader) then
				Spring.Log(LOG_SECTION, LOG.ERROR, glGetShaderLog())
				Spring.Log(LOG_SECTION, LOG.ERROR, "gfx_deferred_rendering.lua: Bad depth beam shader, removing self.")
				GLSLRenderer = false
				widgetHandler:RemoveWidget()
			else
			...
			if options.enableHDR.value and options.enableBloom.value then
				brightShader = brightShader or glCreateShader({
					fragment = VFS.LoadFile("shaders/bloom_bright.fs", VFS.ZIP),
				...
				if not brightShader then
					Spring.Log(LOG_SECTION, LOG.ERROR, 'Deferred Rendering Widget: brightShader failed to compile!')
					Spring.Log(LOG_SECTION, LOG.ERROR, gl.GetShaderLog())
				end

				blurShaderH71 = blurShaderH71 or glCreateShader({
					fragment = VFS.LoadFile("shaders/bloom_blurH.fs", VFS.ZIP),
				...
				if not blurShaderH71 then
					Spring.Log(LOG_SECTION, LOG.ERROR, 'Deferred Rendering Widget: blurshaderH71 failed to compile!')
					Spring.Log(LOG_SECTION, LOG.ERROR, gl.GetShaderLog())
				end

				blurShaderV71 = blurShaderV71 or glCreateShader({
					fragment = VFS.LoadFile("shaders/bloom_blurV.fs", VFS.ZIP),
				...
				if not blurShaderV71 then
					Spring.Log(LOG_SECTION, LOG.ERROR, 'Deferred Rendering Widget: blueShaderV71 failed to compile!')
					Spring.Log(LOG_SECTION, LOG.ERROR, gl.GetShaderLog())
				end

				if not brightShader or not blurShaderH71 or not blurShaderV71 then
					Spring.Log(LOG_SECTION, LOG.ERROR, 'Deferred Rendering Widget: Failed to create bloom shaders!')
					options.enableBloom.value = false
				else
			...
			if options.enableHDR.value then
				combineShader = combineShader or glCreateShader({
					fragment = VFS.LoadFile("shaders/bloom_combine.fs", VFS.ZIP),
				...
				if not combineShader then
					Spring.Log(LOG_SECTION, LOG.ERROR, 'Deferred Rendering Widget: combineShader failed to compile!')
					options.enableHDR.value = false
					Spring.Log(LOG_SECTION, LOG.ERROR, gl.GetShaderLog())
				else
...
function widget:DrawScreenEffects()
	if not (GLSLRenderer) then
		Spring.Log(LOG_SECTION, LOG.ERROR, 'Removing deferred rendering widget: failed to use GLSL shader')
		widgetHandler:RemoveWidget()
		return
	end
